...
# ════════════════════════════════════════════════════════════
# CONFIGURACIÓN DE LA PÁGINA + IDENTIDAD CCI
# ════════════════════════════════════════════════════════════
st.set_page_config(
...
# CSS personalizado con identidad CCI
st.markdown(f"""
<style>
    /* Tipografía */
    html, body, [class*="css"] {{
...
TIPO_CLIENTE_SCORES = {
    "PROFESIONAL": 0, "NO PROFESIONAL": 0,
}

METAMAP_SCORES = {"approved": 0, "revisado": 2, "declined": 5}

# Pesos Matriz Mayo 2026 (sumar exactamente a 1.0)
WEIGHTS = {
    "pep":              0.0,       # PEP=SI → Alto directo, no pondera
    "tipo_cliente":     0.028571,
    "apertura":         0.028571,
    "cond_laboral":     0.095238,
    "sector":           0.095238,
    "profesion":        0.095238,
    "pais_origen":      0.0,       # informativo
    "pais_residencia":  0.0,       # informativo
    "provincia":        0.0,       # informativo
    "tiempo_residencia":0.085714,
    "scoring_credito":  0.028571,
    "productos":        0.095238,
    "fecha_nac":        0.028571,
    "patrimonio":       0.042857,
    "activos_liquidos": 0.042857,
    "ingresos":         0.095238,
    "monto_movilizar":  0.095238,
    "perfil_inv":       0.047619,
    "duracion":         0.047619,
    "metamap":          0.047619,
}
...
def score_duracion(years):
    ...
    if years == 0: return 7   # cliente nuevo
...
def calcular_scores(df, umbral_bajo=5.0, umbral_alto=6.0):
    """Calcula scores y nivel de riesgo para todo el DataFrame."""
    ...
    # Scores individuales — siempre devuelven número
...
# ════════════════════════════════════════════════════════════
# COLUMN MAP (ajusta nombres del Excel/DW a internos)
# ════════════════════════════════════════════════════════════
COLUMN_MAP = {
...
# ════════════════════════════════════════════════════════════
# GENERACIÓN DE PDF (Hoja de Resultados imprimible)
# ════════════════════════════════════════════════════════════
def generar_pdf_resultados(cliente, observacion, analista, posicion):
    ...
    body_style = ParagraphStyle("body", parent=styles["Normal"],
        ...
        alignment=TA_LEFT, spaceAfter=6)
    label_style = ParagraphStyle("lab", parent=styles["Normal"],
        fontName="Helvetica-Bold", fontSize=9, textColor=grisM)
...
with st.sidebar:
    ...
    <div style="text-align: center; padding: 1rem 0;">
        <div style="background-color: {CCI_VERDE}; color: white; padding: 0.5rem;
                    border-radius: 4px; font-weight: bold; font-size: 1.3rem;">
            CCI
        </div>
        <p style="color: {CCI_AMARILLO}; font-size: 0.75rem; margin-top: 0.3rem;
                  letter-spacing: 1px;">PUESTO DE BOLSA, S.A.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    fuente = st.radio(
        ...
        help="DW conectado para producción, archivo para pruebas",
    )
    ...
    if fuente == "Subir archivo Excel":
        ...
        st.info("🔌 Conexión al DW pendiente (Fase 2).\nPor ahora usa la opción de subir Excel.")

    st.markdown("---")
    st.markdown("### ⚙️ Umbrales de clasificación")
    umbral_bajo = st.slider("Bajo ≤", 1.0, 6.0, 5.0, 0.5,
                             help="Clientes con puntaje ≤ este valor → Bajo")
    umbral_alto = st.slider("Alto >", umbral_bajo, 9.0, 6.0, 0.5,
                             help="Clientes con puntaje > este valor → Alto")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "📊 Dashboard", "👥 Clientes Procesados", "⚠️ Alertas",
    "📝 Datos Faltantes", "🖨️ Hoja de Resultados", "📤 Exportar"
])
...
with tab2:
    ...
    with c3:
        if "sucursal" in df.columns:
            ...
            filtro_suc = "Todas"
    with c4:
        busqueda = st.text_input("🔍 Buscar nombre/código")
...
with tab3:
    ...
    if len(alertas) == 0:
        ...
        st.warning(f"⚠️ {len(alertas)} clientes con incoherencia detectada")
